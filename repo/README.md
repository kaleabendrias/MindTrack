...
- All development tooling — including per-run cryptographic secret generation for `./run_tests.sh` — executes inside ephemeral Docker containers; no host-side Node.js, `openssl`, or other crypto tools are ever required
...
Zero-footprint verification — all commands run inside the `test-runner` container. No host-side `jq`, `openssl`, or `uuidgen` required. The stack must be healthy first (`docker-compose ps`).

### Health check

```bash
docker-compose exec test-runner sh -lc 'node -e "
fetch(\"http://127.0.0.1:4000/healthz\")
  .then(r => r.json())
...
### Login (obtain session cookie + csrfToken)

```bash
docker-compose exec test-runner sh -lc 'node -e "
const pass = process.env.SEED_ADMIN_PASSWORD || \"RotateMe_Admin_2026x1\";
fetch(\"http://127.0.0.1:4000/api/v1/auth/login\", {
  method: \"POST\",
  headers: { \"content-type\": \"application/json\" },
  body: JSON.stringify({ username: \"administrator\", password: pass })
}).then(r => r.json()).then(j => console.log(JSON.stringify(j, null, 2)));
"'
# Expected: { "data": { "username": "administrator", "role": "administrator", "csrfToken": "..." } }
```

### Backup status (authenticated GET with HMAC signing)
...
(async () => {
  ...
  const csrf = lj.data.csrfToken;
  const cookie = (lr.headers.getSetCookie() || []).map(c => c.split(\";\")[0]).join(\"; \");
  const path = \"/api/v1/system/backup-status\";
  const ts = Date.now(); const nonce = crypto.randomUUID();
  const sig = crypto.createHmac(\"sha256\", csrf)
    .update([\"GET\", path, ts, nonce, \"\"].join(\"|\")).digest(\"hex\");
  const res = await fetch(BASE + path, {
    headers: { cookie, \"x-signature-timestamp\": String(ts),
               \"x-signature-nonce\": nonce, \"x-signature\": sig }
  });
  console.log(JSON.stringify(await res.json(), null, 2));
})();
"'
# Expected: { "data": { "schedule": "0 0 * * *", "retentionDays": 30, ... } }
```

### Offline policy (unauthenticated)

```bash
docker-compose exec test-runner sh -lc 'node -e "
fetch(\"http://127.0.0.1:4000/api/v1/system/offline-policy\")
  .then(r => r.json())
  .then(j => console.log(JSON.stringify(j, null, 2)));
"'
# Expected: { "data": { "offline": true, "externalNetworkEnabled": false, ... } }
```
